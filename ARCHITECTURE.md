...
## Machine-readable schemas and structured errors

Step 27 is the agent-readiness step, and it is two properties rather than a
feature. Every executable request has a schema that came from the code, and every
expected failure has the same machine-readable shape wherever it surfaces.

**Schemas are generated, committed and diffed**, like the OpenAPI document and
the reference pages.
...
`docs/schemas.py` renders one JSON Schema per request and per object a request is
built out of — `fit-request`, `forecast-request`, `backtest-request`,
`model-recipe`, `fit-plan`, `output-spec`, `time-series-schema`,
`model-descriptor`, `error` — from the Pydantic types themselves, so a renamed
field is a diff rather than a schema that disagrees with what the command
accepts. The documents are a pure function of the types: no catalog is listed and
no provider is started, so two machines with different providers installed
produce the same bytes.

The three requests are the *config* models of `commands/config.py` rather than
the HTTP bodies of `server/wire.py`, which is not a second protocol: the two
differ only in how bulk data travels, the nested objects are literally the same
types, and the HTTP bodies are already described in full by the generated
OpenAPI document. What `spec/schemas` adds is the surface OpenAPI cannot
describe — including a backtest, since HTTP has no backtest endpoint.

`openforecast schema fit --json` prints the same document from the installed
build, which closes the loop an agent actually walks: inspect the schema,
construct the request, execute it. No guessing at a signature, and no schema
pinned to a version that is not the one installed.

**One error protocol, three fields, every boundary.**
...
`OpenForecastError.as_json()` is the one function that shape comes from, so the
CLI's `--json` failure, the HTTP body and `error.code` in Python cannot drift
apart; a provider answers with the same three fields over its own wire protocol,
and `SubprocessProvider` carries its `details` across unchanged, as `HttpTransport`
does across a socket. The CLI writes it to **stderr** with an empty stdout, which
is not a compromise between the two rules but both of them: stdout carries the
requested output, and a failure is the exit code.

**Codes are declared, not derived.** `errors.py` states the code on each class
and `tests/unit/test_errors.py` freezes the table, because a code is what an
agent branches on to decide whether to install a provider, drop a feature, fit
first or give up — and a code derived from a class name would make renaming a
class a breaking protocol change. The finer codes 27.4 asks for arrived as
*subclasses* of the errors that already existed — `UnsupportedDataShape`,
`UnsupportedFeature` and `UnsupportedOutput` under `DataError`,
`InvalidModelParameters` under `RecipeError`, `ProviderNotInstalled` under
`ProviderError` — so no `except` clause anybody had written narrowed, and the
code is what tells the cases apart.
...
docs/llms.txt               generated from the nav, committed, diffed in CI
docs/{getting-started,guides,agents,concepts,integrations}/   written by hand
```
...
The JSON Schemas under `spec/schemas` are the same rule aimed at a machine
rather than a person, and they are generated by the module next door:
`uv run generate-schemas`, committed, and `git diff --exit-code spec/schemas` in
CI.

The documentation has a second *interface* for a reader that arrives with a fetch
tool: `/llms.txt` is the map of the site, `/llms-full.txt` is the whole corpus in
one response, and the Markdown source of every page is served beside its HTML, so
nothing has to be recovered from a themed page. All three are generated by
`uv run generate-llms` and by the `hooks:` entry that calls the same module
during a site build — read off the nav and the first sentence of each page, so a
page that is added, moved or retitled moves in the map too. A curated index would
be accurate on the day it was written, which is the worst kind of index for a
reader that cannot see the site to check it.
