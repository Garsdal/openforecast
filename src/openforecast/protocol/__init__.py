...
each other. ``ForecastColumn`` is written by whoever executes a model and read
by the engine that asked for it. ``PROTOCOL_VERSION`` is stamped into an
artifact manifest in Step 7 and negotiated over the handshake in Step 9, and
those two have to be one number.
"""
...
__all__ = ["ForecastColumn", "PROTOCOL_VERSION", "ViewKind", "forecast_columns"]
