"""``DartsProvider``: the three calls, dispatched to an adapter.
...
provider and the Nixtla integration implement, which is what lets this run in a
subprocess in its own environment without the engine knowing. There is no
branching here on where the data came from, because there is nothing to branch
on: what arrives is an execution view.
"""
...
from openforecast_darts import catalog
from openforecast_darts._version import __version__

__all__ = ["PROVIDER_NAME", "PROVIDER_VERSION", "DartsProvider"]

#: Also the namespace of every model advertised: ``darts/tide``.
PROVIDER_NAME = "darts"
...
class DartsProvider:
    """Executes Darts' models, in whatever process it was started in."""
    ...
    def __repr__(self) -> str:
        return f"DartsProvider(version={self.version}, models={len(catalog.model_names())})"
