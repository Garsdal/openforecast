...
Every scikit-learn estimator takes a different bag of keyword arguments, and a
caller needs two things from an integration about each of them: what may be
passed, and why what they passed was refused. Declaring the parameter once and
deriving both from it is what stops the JSON Schema in the descriptor and the
validation in the adapter from drifting apart — a parameter that is documented is
one that is accepted, and a parameter that is refused is refused by name.

Shared by the adapters rather than living with one of them, because the next
estimator this integration exposes should be a table of parameters and nothing
else. If adding ``sklearn/ridge`` needs more than that, the ``TabularView``
boundary is doing less work than it claims to.

The same declaration the Nixtla, Darts and sktime integrations make, for the
same reason: a provider owns the spelling of its own parameters, and there is no
shared surface for them to be declared on without OpenForecast learning what a
learning rate is.
"""
