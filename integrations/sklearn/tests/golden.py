...
something a supervised model with a feature that also moves with the event time
tracks closely, so an assertion can be arithmetic rather than a recorded number,
and a forecast that came back for the wrong event times or the wrong instance is
...
repository. They are deliberately the same builders the Nixtla, Darts and sktime
integrations write their tests against — the point is that the same data, the
same plans and the same assertions hold for a fourth library, and the only thing
that changes is that this one is handed rows rather than series.
"""
...
#: Enough boosting to prove the wiring, and no more. These tests assert what the
#: estimator was *given* and how the answer is *labeled*; how well a hundred
#: trees fit a handful of rows is not something a unit test can assert without
#: becoming a benchmark, and paying for them to find out would make the suite
#: slow for nothing.
FAST = {"max_iter": 5}

#: What a feed that had not published yet at an origin leaves in a row.
NAN = math.nan
...
def event_time_frame(
    *,
    ...
    ``known`` defaults to ``True`` here where the other integrations default it
    to ``False``: a supervised row is built from what was knowable at its origin,
    so a frame with no feature at all has no tabular view to materialize — which
    is a property of the view rather than of this provider, and is asserted as
    such in ``test_hist_gradient_boosting.py``.
    """
...
def point_in_time_dataset(
    *,
    ...
    vintage is identifiable rather than merely suspicious — which is what makes
    "the row of origin 4 holds the values of origin 4" an assertion about
    arithmetic rather than about a recorded blob.

    ``published_from`` is the origin step at which the known feed starts
    publishing. Before it, the feature is missing for every event time, which is
    the shape point-in-time data actually has and the reason this integration's
    first estimator is one that reads ``NaN`` natively.
    """
        ...
        for step in range(origins):
            ...
            for event in range(origin + horizon + 1):
                ...
                if known:
                    row[KNOWN] = (
                        NAN
                        if published_from is not None and step < published_from
                        else known_value(index, event, origin)
                    )
                if observed:
                    row[OBSERVED] = observed_value(index, event) if event <= origin else NAN
                if static:
                    row[STATIC] = static_value(index)
                rows.append(row)
