...
and the Darts integration write their tests against — the point of Step 14 is
that the same data, the same plans and the same assertions hold for a third
library, so the fixtures are the same too.
"""
...
#: model was *given* and how the answer is *labeled*; how well a hundred trees
#: fit six-step windows is not something a unit test can assert without becoming
#: a benchmark, and paying for them to find out would make the suite slow for
#: nothing.
FAST = {"max_iter": 5}
