...
applied to Step 14's provider: the same model, fitted and forecast in this
...
def test_a_sequence_view_survives_the_wire_with_its_sample_boundaries(
    ...
    a sample whose bounds were lost is a training sequence nobody described, and
    a static feature that was lost is a column of the panel that never existed.
    """
...
def test_a_failure_inside_the_provider_arrives_as_the_error_it_is(
    ...
    with pytest.raises(RecipeError, match="sp"):
        client.fit(THETA, golden.event_time_frame(periods=24), params={"sp": 0})
