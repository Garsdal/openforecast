"""``SktimeProvider``: the three calls, dispatched to an adapter.
...
provider and the Nixtla and Darts integrations implement, which is what lets
this run in a subprocess in its own environment without the engine knowing.
...
from openforecast_sktime import catalog
from openforecast_sktime._version import __version__

__all__ = ["PROVIDER_NAME", "PROVIDER_VERSION", "SktimeProvider"]

#: Also the namespace of every model advertised: ``sktime/pooled-trees``.
PROVIDER_NAME = "sktime"
...
class SktimeProvider:
    """Executes sktime's forecasters, in whatever process it was started in."""
    ...
    def __repr__(self) -> str:
        return f"SktimeProvider(version={self.version}, models={len(catalog.model_names())})"
