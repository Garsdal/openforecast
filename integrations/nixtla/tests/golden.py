...
something an ARIMA continues exactly, so an assertion can be arithmetic rather
than a recorded number, and a forecast that came back for the wrong event times
or the wrong instance is visible in the value itself.
...
def target_value(instance: int, event: int) -> float:
    """A straight line per instance, which an ARIMA continues exactly."""
