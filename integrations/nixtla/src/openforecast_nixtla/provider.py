"""``NixtlaProvider``: the three calls, dispatched to an adapter.
...
provider implements, which is what lets this run in a subprocess in its own
environment without the engine knowing. There is no branching here on where the
data came from, because there is nothing to branch on: what arrives is an
execution view.
"""
...
from openforecast_nixtla import catalog
from openforecast_nixtla._version import __version__

__all__ = ["PROVIDER_NAME", "PROVIDER_VERSION", "NixtlaProvider"]

#: Also the namespace of every model advertised: ``nixtla/autoarima``.
...
class NixtlaProvider:
    """Executes Nixtla's models, in whatever process it was started in."""
    ...
    def __repr__(self) -> str:
        return f"NixtlaProvider(version={self.version}, models={len(catalog.model_names())})"
