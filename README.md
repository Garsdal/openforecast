...
Every error carries the same three fields — a stable `error.code`, here
`MODEL_DOES_NOT_SUPPORT_FIT`, the `error.message` a person reads, and the
`error.details` behind it — so a caller branches on the failure rather than on
its prose.
...
Nothing about using OpenForecast should require reverse-engineering a Python
signature. Every request has a JSON Schema, generated from the model that
validates it:
...
The documents are committed under `spec/schemas` — `fit-request.json`,
`forecast-request.json`, `backtest-request.json`, `model-recipe.json`,
`fit-plan.json`, `output-spec.json`, `time-series-schema.json`,
`model-descriptor.json`, `error.json` — generated by `uv run generate-schemas`
and diffed in CI, exactly like the OpenAPI document. None of them is written by
hand, so a renamed field is a diff rather than a schema that quietly disagrees
with what the command accepts.

Failure is one protocol, at every boundary:

```python
try:
    of.forecast(model="nixtla/nhits", data=data, horizon=24)
...
The same three fields come back from `openforecast ... --json` on stderr, from
the HTTP API as `{"error": {...}}`, and from a provider over its own wire
protocol. The codes are declared and frozen by a test —
`MODEL_NOT_FOUND`, `MODEL_REQUIRES_FIT`, `MODEL_DOES_NOT_SUPPORT_FIT`,
`UNSUPPORTED_DATA_SHAPE`, `UNSUPPORTED_FEATURE`, `UNSUPPORTED_OUTPUT`,
`INVALID_DATA`, `INVALID_MODEL_PARAMETERS`, `PROVIDER_NOT_INSTALLED`,
`PROVIDER_EXECUTION_FAILED` and the rest — because an agent recovers on the code,
...
uv run generate-schemas   # regenerate spec/schemas
```
...
src/openforecast/
    ...
    docs/        the reference-documentation and JSON Schema generators
